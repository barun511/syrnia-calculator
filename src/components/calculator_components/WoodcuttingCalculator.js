...
export default class WoodcuttingCalculator extends React.Component {
  ...
  handleLevelChange(event) {
    this.setState({
      player_level: Number(event.target.value),
    });
  }
  ...
  render() {
    ...
    return (
      <div className="calculator">
        ...
        <div className="timer">
          {this.state.forest_level <= this.state.player_level ? "Base timer : " + calculate_woodcutting_timer(this.state.base_forest_timer, this.state.tool_reduction, this.state.player_level) : "You probably can't chop here yet."}
        </div>
      </div>
    )
  }
